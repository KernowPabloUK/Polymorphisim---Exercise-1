...
namespace Polymorphisim___Exercise_1
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string serverType = "Oracle";
            string server = "localhost";
            string database = "MyDB";
            string instruction = "bla bla bla";

            DbConnection dbConnection = CreateDBConnection(serverType, server, database);
            DbCommand dbCommand = new DbCommand(dbConnection, instruction);
            dbCommand.Execute();
        }

        private static DbConnection CreateDBConnection(string serverType, string server, string database)
        {
            DbConnection dbConnection;
            if (serverType == "SQL") dbConnection = new SqlConnection(server + ";" + database);
            else if (serverType == "Oracle") dbConnection = new OracleConnection(server + "-" + database);
            else throw new Exception("Servertype unknown");
